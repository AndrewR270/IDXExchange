export default function PropertyCard({ property }: { property: any }) {
  ...
  // Get first parsed photo in L_Photos, if present
  let photoUrl = "";
  try {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:scale-[1.02] transform transition-all duration-200 flex flex-col">
      ...
      <div className="p-4 flex flex-col gap-1">
        <h3 className="text-lg font-semibold text-gray-900">${property.L_SystemPrice?.toLocaleString()}</h3>
        <p className="text-gray-700">{property.L_Address}</p>
        <p className="text-gray-600">{property.L_City}, {property.L_State}</p>
        <p className="text-gray-500 text-sm mt-2">{property.L_Keyword2} beds • {property.LM_Dec_3} baths • {property.LM_Int2_3} sqft</p>
      </div>
    </div>
  );
}
